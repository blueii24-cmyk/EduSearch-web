...
export default function ProfileWizard({ profile, onSave }) {
  ...
  const next = () => { if (step < profileSteps.length - 1) setStep(step + 1); else { onSave(draft); setSaved(true) } }
  return <div className="profile-form-area"><div className="profile-form-top"><span className="eyebrow">{current.label}</span><span className="save-note">Saved locally <span className="save-dot" /></span></div><div className="profile-question" key={step}><h2>{current.title}</h2><p>{current.copy}</p>{current.options && <div className="selection-grid">{current.options.map((option) => { const selected = Array.isArray(draft[current.field]) ? draft[current.field].includes(option) : draft[current.field] === option; return <button type="button" key={option} className={`selection-option ${selected ? 'selected' : ''}`} onClick={() => current.field === 'skills' || current.field === 'interests' ? toggle(option) : update(option)}><span className="option-mark">{selected ? <Check size={15} /> : '+'}</span>{option}</button> })}</div>}{current.type === 'number' && <div className="number-input-wrap"><input autoFocus type="number" min="0" max="100" value={draft.percentage} onChange={(event) => update(event.target.value)} /><span>%</span></div>}{current.type === 'location' && <div className="location-input-wrap"><MapPin size={19} /><input autoFocus value={draft.location} onChange={(event) => update(event.target.value)} placeholder="e.g. Panaji, Goa" /></div>}</div><div className="profile-form-footer"><button className="button button-ghost" onClick={() => step > 0 && setStep(step - 1)} disabled={step === 0}><ChevronLeft size={17} /> Back</button><button className="button button-primary" onClick={next}>{step === profileSteps.length - 1 ? saved ? 'Profile saved' : 'Finish profile' : 'Continue'}{step < profileSteps.length - 1 && <ChevronRight size={17} />}</button></div>{saved && <div className="success-note"><Check size={16} /> Your profile is ready. We’ll use it to personalize your matches.</div>}</div>
}
